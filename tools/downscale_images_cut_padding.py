...
PADDING_LEFT = 40 # cut off left marigin
PADDING_RIGHT = 10 # cut off right marigin
...
def parse_args():
  ...
  parser.add_argument('-target_size', type=int, default=127,
                      help='A minimum size of a image side (weight or height)')
  return parser.parse_args()


def downscale(paths: list, min_size: int, save_aspect_ratio: bool, save_dir: str):
  """ Downscale images and saves it to dir with -downscale enging. Also cutting off PADDING_LEFT and PADDING_RIGHT
  ...
  def _resize_and_save(path, img, target_width, target_height):
    ...
    img = img[0:height, PADDING_LEFT:(width-PADDING_RIGHT)]; # img[y:y+h, x:x+w]
  ...
  for id_, key in enumerate(paths):
    ...
    if save_aspect_ratio:
      if min_size in (width, height):
        ...
        _resize_and_save(save_path, img, target_width, target_height)
      ...
      _resize_and_save(save_path, img, min_size, min_size)
...
def main():
  ...
  for (dirpath, _, filenames) in os.walk(root_dir):
    ...
    downscale(filenames, min_size=args.target_size, save_aspect_ratio=False, save_dir=save_dir)
